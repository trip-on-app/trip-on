...
관리자 화면의 프로모션 코드는 관리자 Worker의 `admin_promotions`와 앱 Worker가 확인하는
`promo_codes`를 하나의 D1 배치로 함께 저장합니다. 플랜, 이용 기간(1~365일), 사용 인원 제한,
코드 만료일이 앱 코드 검증에 즉시 반영됩니다. 수정 시 기존 코드명을 바꾸면 이전 코드는
비활성화됩니다. 기존 관리자 코드는 배포 마이그레이션에서 Pro/Enterprise 플랜과 30일
이용 정책으로 운영 프로모션 테이블에 가져옵니다.
